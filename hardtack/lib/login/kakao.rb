...
module Kakao
  ...
  def self.login(kakao_access_token)
    ...
    user = User.find_by_service_and_identifier(::Services::KAKAO, id)
    user = User.create(
      {nickname: nickname, service: ::Services::KAKAO, identifier: id}
    ) if user.nil?

    access_token = ::HardtackAuth.encrypt(user.id.to_s)
    #Rails.cache.write("session:#{access_token}", user.id, expires_in: 12.hours)
    Rails.cache.write("session:#{access_token}", user.id, expires_in: 5.minutes)
    access_token
  end
end
